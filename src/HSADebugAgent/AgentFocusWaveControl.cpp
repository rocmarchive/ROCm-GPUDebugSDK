...
HsailAgentStatus AgentFocusWaveControl::GetFocus(HwDbgDim3& opWg, HwDbgDim3& opWi) const
{
    ...
    else if (CompareHwDbgDim3(m_focusWorkItem, gs_UNKNOWN_HWDBGDIM3))
    {
...
//
// If input Dim3 params are unknown, choose the first entry from the waveinfo buffer
//
                                                     ...
                                                     const HwDbgDim3* pWg, const HwDbgDim3* pWi)
{
    ...
    {
        AGENT_ERROR("SetFocusWave: DBE handle is nullptr");
        return status;
    }
    ...
    {
        AGENT_ERROR("SetFocusWave: WaveInfo buffer is invalid");
        return status;
    }

    if (isBufferEmpty)
    {
        AGENT_LOG("SetFocusWave: Empty wave info buffer, we cannot change the focus");
    ...
    if (CompareHwDbgDim3(m_focusWorkGroup, gs_UNKNOWN_HWDBGDIM3) ||
        CompareHwDbgDim3(m_focusWorkItem, gs_UNKNOWN_HWDBGDIM3))
    {
        AGENT_LOG("SetFocusWave: Choose the first wave in the wave info buffer to set focus");
    ...
    // The focus wg and wi are known, we try and find it in the wave info buffer
    // If we do find it, we dont change the focus wave
    ...
    // found in the above search of the waveinfo buffer.
    // If the focus WI was not found, change it and let gdb know
