...
HsailAgentStatus AgentFreeSharedMemBuffer(const key_t shmkey, const size_t maxShmSize)
{
    ...
    if (pShm == (int*) - 1)
    {
        AGENT_ERROR("FreeSharedMemoryBuffer: Error with shmat, "
                    "could not map ID with key: "<< shmkey  << " Max size: " << maxShmSize);
    ...
    if (shmdt(pShm) == -1)
    {
        AGENT_ERROR("FreeSharedMemoryBuffer: shmdt failed, "
                    "could not detach key: "<< shmkey  << " Max size: " << maxShmSize);
...
void* AgentMapSharedMemBuffer(const key_t shmkey, const size_t maxShmSize)
{
    ...
    if (shmid  < 0)
    {
        AGENT_ERROR("AgentMapSharedMemBuffer: Error with shmget, "
                    "could not get ID with key: "<< shmkey  << " Max size: " << maxShmSize);
    ...
    if (pShm == (int*) - 1)
    {
        AGENT_ERROR("AgentMapSharedMemBuffer: Error with shmat, "
                    "could not map with key: "<< shmkey  << " Max size: " << maxShmSize);
...
HsailAgentStatus AgentAllocSharedMemBuffer(const key_t shmkey, const size_t maxShmSize)
{
    ...
    if (shmdt(pShm) == -1)
    {
        AGENT_ERROR("AllocateMomentaryBPBuffer: shmdt failed, "
                    "could not detach with key: "<< shmkey  << " Max size: " << maxShmSize);;
    }
    else
    {
        status = HSAIL_AGENT_STATUS_SUCCESS;
